...
Wombat::Device::Device()
    ...
    connect(
        m_settingsProvider, SIGNAL(settingsChanged()), SLOT(settingsChanged()));
...
QString Wombat::Device::name() const { return tr("Wombat"); }

QString Wombat::Device::version() const { return m_version; }

QString Wombat::Device::id() const { return m_id; }

QString Wombat::Device::serial() const { return m_serial; }
...
// TODO: Connect setting provider's signal to a battery provider slot that will
// load settings
void Wombat::Device::settingsChanged() {
    const int type = m_settingsProvider->value("battery_type", 0).toInt();
    const float thresh =
        m_settingsProvider->value("battery_warning_thresh", 0.1f).toFloat();
    const bool enabled =
        m_settingsProvider->value("battery_warning_enabled", true).toBool();

    Wombat::BatteryLevelProvider *wblProvider =
        (Wombat::BatteryLevelProvider *)m_batteryLevelProvider;
...
void Wombat::Device::timerEvent(QTimerEvent *event) {
    ...
    if (batteryLevel < warningThresh) {
        ...
        if (std::chrono::duration<double>(now - last_warn_time).count() >
            WAV_CYCLE_TIME)
        {
