...
Compiler::OutputList KissCompileProvider::compile(const QString &name, const kiss::KarPtr &program, const QString &install_dir)
{
	if (program.isNull())
	{
	...
	qDebug() << "installing to kipr project dir by copying from " << output << " to " << dest_file;
