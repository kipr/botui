...
BatterySettingsWidget::BatterySettingsWidget(Device *device, QWidget *const parent)
  ...
  , m_numpadInput(new NumpadDialog("Warning Threshold", NumpadDialog::Integer, 0, 100, this))
{
  ui->setupUi(this);
  ...
  if(settingsProvider) {
    ...
    const float warnThresh = settingsProvider->value(BATTERY_WARNING_THRESH, 0.1f).toFloat();
  ...
  connect(ui->warningEnabled, SIGNAL(stateChanged(int)), SLOT(warnEnabledChanged(int)));
  connect(ui->warningThresh, SIGNAL(textEdited(QString)), SLOT(warnThreshChanged(QString)));
}
