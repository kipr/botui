...
const QColor &QHsvPicker::max() const
{
	return m_max;
}
...
const QColor &QHsvPicker::min() const
{
	return m_min;
}
...
void QHsvPicker::paintEvent(QPaintEvent *)
{
	...
	drawDragHint(painter, selS.bottomRight(), Qt::black, m_activeS == BottomRight);
	
	drawDragHint(painter, selV.topLeft(), Qt::white, m_activeV == TopLeft);
	drawDragHint(painter, selV.bottomRight(), Qt::white, m_activeV == BottomRight);
...
void QHsvPicker::mousePressEvent(QMouseEvent *event)
{
	...
	if(isDragRange(pos, selS.topLeft())) m_activeS = TopLeft;
	else if(isDragRange(pos, selS.bottomRight())) m_activeS = BottomRight;
	else if(selS.contains(pos)) m_activeS = All;
	if(m_activeS != None) return; // Disable mutliple selection for now
	
	if(isDragRange(pos, selV.topLeft())) m_activeV = TopLeft;
	else if(isDragRange(pos, selV.bottomRight())) m_activeV = BottomRight;
	else if(selV.contains(pos)) m_activeV = All;
...
QRect QHsvPicker::selectionS() const
{
	...
	return QRect(QPoint(qMin(selMinH, selMaxH) * widthRatio, startY * heightRatio),
...
QRect QHsvPicker::selectionV() const
{
	...
	return QRect(QPoint(qMin(selMinH, selMaxH) * widthRatio, height() / 2.0f + startY * heightRatio),
		QPoint(qMax(selMinH, selMaxH) * widthRatio, height() / 2.0f + endY * heightRatio));
}
...
void QHsvPicker::drawDragHint(QPainter &painter, const QPoint &p, const QColor &c, const bool &active) const
{
	painter.setPen(c);
	int r = dragHintRadius();
	if(active) r *= 2;
	painter.setBrush(active
		? QColor(c.red(), c.green(), c.blue(), 127)
...
const unsigned &QHsvPicker::slideTimeout() const
{
	return m_slideTimeout;
}
...
void QHsvPicker::scheduleSlide()
{
	m_slideTimer->start(m_slideTimeout);
}
