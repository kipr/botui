...
SerialIODevice::SerialIODevice(const QString &path)
    : m_path(path), m_baudRate(115200) {
    qDebug() << "Created serial device at" << m_path << "with BR" << m_baudRate;
}

SerialIODevice::~SerialIODevice() { close(); }
...
quint32 SerialIODevice::baudRate() const { return m_baudRate; }
...
qint64 SerialIODevice::writeData(const char *data, qint64 maxSize) {
    ...
    if (m_fd == INVALID_HANDLE_VALUE) return -1;
    DWORD write_count;
    return !WriteFile(m_fd, data, maxSize, &write_count, NULL) ? 0
                                                               : write_count;
#else
    if (m_fd <= 0) return -1;
